...
##### `detect_profile_orientation(kps_5, threshold=0.3) -> str`

**Purpose**: Automatically detect if a face is in profile and its orientation.
...
4. Classify as 'left_profile', 'right_profile', or 'frontal'

**Parameters**:
- `kps_5`: numpy array of 5-point landmarks [left_eye, right_eye, nose, left_mouth, right_mouth]
- `threshold`: sensitivity for profile detection (0.0-1.0)

**Returns**: String indicating orientation

**Tuning**:
- Lower threshold (0.2): Detects subtle profiles
- Higher threshold (0.5): Only detects strong profiles
- Default (0.3): Balanced detection

---

##### `adjust_profile_keypoints(kps_5, profile_side, enhancement=0.5) -> np.ndarray`

**Purpose**: Adjust keypoints for better profile face alignment.

**Algorithm**:
1. Identifies occluded landmarks based on profile side
2. Estimates better positions for occluded features
3. Applies enhancement-weighted adjustments
...
##### `create_profile_mask(profile_side, enhancement=0.5, device='cuda') -> torch.Tensor`

**Purpose**: Generate asymmetric mask for profile blending.

**Algorithm**:
1. Create base meshgrid (128x128)
2. Calculate distance from center for each pixel
3. Apply side-specific weighting:
   - Boost visible side: `1.0 + enhancement * 0.3`
   - Reduce occluded side: `1.0 - enhancement * 0.2`
4. Create smooth falloff with weighted distance
...
##### `apply_profile_color_correction(swap, original, profile_side, enhancement=0.5) -> torch.Tensor`

**Purpose**: Apply profile-specific color matching.

**Algorithm**:
1. Split image at midpoint
2. Extract visible region based on profile side
3. Calculate color statistics (mean, std) for visible region
4. Apply color transfer: `normalized = (swap - swap_mean) / swap_std`
5. Re-scale: `corrected = normalized * orig_std + orig_mean`
6. Blend with original based on enhancement

**Color Transfer Strength**: `enhancement * 0.3`
...
##### `get_profile_border_adjustments(profile_side, enhancement=0.5) -> dict`

**Purpose**: Calculate border mask adjustments for profiles.
...
**Logic**:
- Extends border on occluded side: `10 + enhancement * 15`
- Reduces border on visible side: `10 - enhancement * 5`
